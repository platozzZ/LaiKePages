...
export async function app_pay(me,type){
	// alert('app_pay-type:' + type)
	let orderInfo = await me.getOrderInfo(type)
	...
	me.$bridge.call("cmmc.pushWXPay", orderInfo.data, (data) => {
		...
		if(newdata.resultStatus=='9000'){
			uni.showToast({
				...
				success() {
					...
					setTimeout(function() {
						...
						me._uni_redirectTo("../../pages/order/myOrder")
			...
			me._uni_redirectTo("../../pages/order/myOrderr")
			// alert('微信支付失败')
			// uni.redirectTo({
	...
	if (me.form_type == 1) {
		...
		if (me.wx_flag == true && me.ye_flag == false) {
			...
			if (!me.zfb_flag) {
				...
				// alert(uni.getStorageSync('tripartiteChannel') == 'cmmc')
