	...
	export default {
		...
		onShow() {
			...
			if (this.code == '') {
				...
				console.log('要走入跳转')
				// if(uni.getStorageSync('tripartiteChannel') == 'cmmc'){
				// 	this.toUrl();
				// }
		...
		methods: {
			...
			getOrderInfo(type) {
				...
				return p.then((authcode) => {
					...
					// alert('getOrderInfo-type:' + JSON.stringify(data))
					return new Promise((res) => {
						uni.request({
							url: uni.getStorageSync("url"),
							data,
							success: (result) => {
								// alert('getOrderInfo-result:' + JSON.stringify(result))
			 ...
			 * 微信支付和支付宝支付（非小程序）
			 * */
			async pay_wx(type) {//app_pay
				// alert('pay_wx-type:' + type)
				if(type == 'app_wechat'){
					app_pay(this,type)
				} else {
...
<style scoped>
	...
	.bid_footer{
		...
		height: calc(110upx + constant(safe-area-inset-bottom) / 2);
		height: calc(110upx + env(safe-area-inset-bottom) / 2);
