<script>
	...
	export default {
		...
		onLaunch: function (options) {
			...
			if(channel=='cmmc'){
				me.$store.state.channel = channel
				uni.setStorageSync('tripartiteChannel',channel)
				me.$loginFrom._loginH5(me)
			    //获取手机状态高度
			    var nate = me.$bridge.call("cmmc.getStatusBar");
				// alert(nate)
				uni.setStorageSync('data_height',nate)
				me.data_height(nate)
				let botHeight = me.$bridge.call("cmmc.getBottomSafeAreaHeight")
				if(botHeight > 0){
					uni.setStorageSync('isiPhoneX',botHeight)
				}
			}
			// 导航信息条高度
			// uni.getSystemInfo({
